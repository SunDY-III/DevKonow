...
当用户向 DevKnow 提问时，系统内部经过以下完整链路：

```
用户提问："OrderService 的 createOrder 方法是怎么实现的？"
       │
       ▼
┌──────────────────────────────┐
│  ① 路由分类（fastModel）      │
│  code / doc / both / unknown │
└──────┬───────┬───────┬───────┘
       │       │       │
  ┌────┘  ┌────┘  ┌────┘
  ▼       ▼       ▼
code    doc    both/unknown → ReAct Agent
 │       │            │
 │       │       ┌────┴──────────────┐
 │       │       │  ② 推理循环         │
 │       │       │  Thought→Action→   │
...
│                                      │
│  Agent 探索轮（轻量）：                │
│  向量 TopK=4 + 关键词 TopK=4 → RRF   │
│                                      │
│  Agent 确认轮（全量）：                │
│  ┌─ HyDE 假设文档生成                  │
│  ├─ 稠密向量搜索（Qdrant）            │
│  ├─ 稀疏权重搜索（LLM×MySQL ngram）    │
│  ├─ 关键词搜索（MySQL ngram 同义词）    │
│  ├─ 三路 RRF 融合                     │
│  ├─ MMR 多样性去重                    │
│  ├─ Cross-encoder 重排               │
│  ├─ Neo4j 知识图谱扩展                │
